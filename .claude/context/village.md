...
| Projects | A long job split into tasks a house takes, or is handed by the task's creator after agreeing in person. A project is planned, then in progress, then finished, and only those in progress count on the Home badge. Events belong to a project and show a 📋 chip in the calendar. A strip of pictures — the fence before, during, after — that any house adds to, because the chat loses a photo in a week. Finished stays visible. On the phone's bottom bar. |
...
| Codex | The founding text the collective hands a house before it joins — values, the way we speak to each other, the agreements on animals, noise, meals, invasive species. On paper it is signed once and never seen again; here it is a chip at the head of Home, beside the map and the weather, in the village's languages, and a house amends it in place with its name on the change. A steward imports the adopted text once from a file; the repo never carries it. |
| Contacts | "Who did we call about the well last time?" A name, a number, a note and a type — the well-driller, the vet, the chimney sweep, the municipal office. Any house writes one down and any house corrects a wrong number, and the card names whoever last did — the number is the village's, not a house's. The type is a free word, so the trades the village actually calls are whatever the houses wrote. Nothing rings: a number is looked up, not announced. A tile on Home after Houses. |
| Changelog | "Is the thing I asked for in yet?" The builder's commit subjects of the last 90 days, grouped by day, written at build time from the git log. A panel instead of a message to the builder, behind a chip at the foot of Home. |
| MCP door | "Tell my assistant, and it is in the portal." A house mints a key at the foot of the Houses room, and its agent — Claude Code or any client that speaks HTTP MCP — acts as that house through the same handlers, with the same pushes: the work party on the calendar, the shop need, the codex section read aloud. For the owner's agent and for any villager who runs one; a house that runs none sees nothing change. The Watchtower is on it by the owner's line: a key is the logged-in house, and the provider is a third party the house chose, like WhatsApp; the phone book joined it on 2026-09-10. |
