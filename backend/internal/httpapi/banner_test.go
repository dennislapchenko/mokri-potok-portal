...
func TestNotificationBanners(t *testing.T) {
	...
	_, o, _ := steward.do("POST", "/api/houses", map[string]any{"name": "House Zeleni Volk"})
	volk := &client{t: t, h: h}
	_, j, _ := volk.do("POST", "/api/join", map[string]any{"code": o["invite"].(map[string]any)["code"]})
	volk.token = j["token"].(string)
	...
	cases := []struct {
		...
		{"need", "POST", "/api/needs",
			...
			[2]string{"🛒 House Zeleni Volk needs from the shop", "mleko 2l in kvas"}},
		{"offer", "POST", "/api/offers",
			map[string]any{"text": "stara okna, 4 kosi", "tag": "giveaway"},
			[2]string{"🎁 House Zeleni Volk podarja", "stara okna, 4 kosi"},
			[2]string{"🎁 House Zeleni Volk gives away", "stara okna, 4 kosi"}},
		{"seeds", "POST", "/api/offers",
			map[string]any{"text": "paradižnik, semena", "tag": "seeds"},
			[2]string{"🌱 House Zeleni Volk deli semena", "paradižnik, semena"},
			[2]string{"🌱 House Zeleni Volk shares seeds", "paradižnik, semena"}},
		{"post", "POST", "/api/posts",
			map[string]any{"body": "wasap zvečer", "author": "Ana"},
			[2]string{"🍺 Ana · House Zeleni Volk v gostilni", "wasap zvečer"},
			[2]string{"🍺 Ana · House Zeleni Volk in the tavern", "wasap zvečer"}},
		...
		{"project", "POST", "/api/projects",
			map[string]any{"title": "Ograja okoli travnika", "notes": "300 m"},
			[2]string{"📋 House Zeleni Volk načrtuje: Ograja okoli travnika", "300 m"},
			[2]string{"📋 House Zeleni Volk plans: Ograja okoli travnika", "300 m"}},
		{"away", "POST", "/api/away",
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			fake.mu.Unlock()
			code, _, _ := volk.do(c.method, c.path, c.body)
			if code != 201 {
			...
			for i, sub := range fake.sent {
				...
				if p.Kind == "away" {
					for _, leak := range []string{"2026-09-10", "2026-09-14", "kokoši", "Zeleni Volk"} {
