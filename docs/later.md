...
| The one-page privacy note, with a line about campers, **and a line about agents**: an away-notice can be read by a neighbour's assistant, as it can by a neighbour's phone. The village was never told, and the argument for putting the Watchtower on the agent surface only works if it is | The collective's board, a lawyer confirms | A new `docs/privacy.md` |
