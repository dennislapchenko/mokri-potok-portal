...
| The tavern's `starts_at >= date('now','-60 days')` in `listEvents` compares a wall-clock column against a bare date, so the shapes differ at the boundary day. The window is 60 days wide and nothing is visibly wrong | The owner, if it ever misbehaves | `CLAUDE.md` § Two clocks |
| `projects.go` has the right shape but still reads SQLite's clock rather than `s.now()`, so no test can move it. Same testability gap `listRuns` had | The owner, when that query next needs a test | `CLAUDE.md` § Two clocks |
