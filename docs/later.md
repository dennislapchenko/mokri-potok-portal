...
| The tavern's `starts_at >= date('now','-60 days')` in `server.go` compares a wall-clock column against a bare date, so the shapes differ at the boundary day. The window is 60 days wide and nothing is visibly wrong | The owner, if it ever misbehaves | `CLAUDE.md` § Two clocks |
